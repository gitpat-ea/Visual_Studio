...
{
    std::ofstream fout( "Dynamic_double_mem.txt", std::ios::app );
    fout << time << ' ' << size << ' ';
...
DoubleMemArr create_array(int size, int increase){
    DoubleMemArr array;
    array.size = size;
    array.fullness = size;
...
void delete_array(DoubleMemArr *array){
    delete[] array->value;
} 

void Print_double_mem(DoubleMemArr &array){
    if(array.fullness == 1){
...
DoubleMemArr array_expancy(DoubleMemArr *array){
    DoubleMemArr newarr;
...
void change_double_mem(DoubleMemArr &array, int value, int place){
...
DoubleMemArr push_double_mem(DoubleMemArr &array, int number){
    if(array.fullness < array.size){
        ...
        DoubleMemArr newarr = array_expancy(&array);
...
int main(){
    for(int j = 100; j < 1000000+1; j = round(j*1.2)){
        DoubleMemArr arr = create_array(1, 2);
        change_double_mem(arr, 0, 0);
        auto begin = std :: chrono :: steady_clock ::now(); 
        for (long int i = 1; i < j; i++){
            arr = push_double_mem(arr, i);
        }
